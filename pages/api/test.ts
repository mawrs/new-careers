...
export default async function handler(
  ...
  if (req.method === 'POST') {
    try {
      ...
      const prompt = `Suggest exactly 25 diverse careers for someone with a major in ${searchInput}. For each career, provide:
        ...
        Format your response as a JSON array with exactly 25 objects, each containing the following fields: 'title', 'description', 'salaryRange', 'growth', 'scarcity', 'education', and 'competition'.
        
        IMPORTANT: Provide ONLY the JSON array with EXACTLY 25 items. Do not include any other text before or after the JSON.`;
      ...
      const response = await fetch(API_URL, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "You are a career advisor AI specializing in connecting academic majors to diverse career opportunities. Always respond with valid JSON containing exactly 25 career items."
            },
            {
              role: "user",
              content: prompt
      ...
      // Ensure we have exactly 25 careers
      if (Array.isArray(careers)) {
        if (careers.length > 25) {
          careers = careers.slice(0, 25);
        } else if (careers.length < 25) {
          throw new Error(`Not enough careers: got ${careers.length}, expected 25`);
        }
      } else {
      ...
      res.status(200).json(careersWithIds);
    } catch (error) {
